...
class Rake(object):
    ...
    def run(self, text):
        ...
        values_list = keyword_candidates.values()
        max_value = max(values_list)
        min_value = min(values_list)
        
        for keyword, value in keyword_candidates.items():
            fit_value = (value - min_value) / (max_value - min_value)
            keyword_candidates[keyword] = fit_value
